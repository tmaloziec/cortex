...
- **"Can a plugin break security?"** → `ARCHITECTURE.md` →
  ...
  think about".
- **"Is CS talk encrypted?"** → `CD_HANDOFF.md` → "Szyfrowanie
  komunikacji Cortex ↔ CS". Short answer: no, plaintext HTTP;
...
- **"Can CS be the hub for multi-agent infrastructure?"** → Yes
  today with caveats; better in v1.1. `CD_HANDOFF.md` → "CS jako
  multi-agent hub".
